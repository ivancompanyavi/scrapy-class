...
class SerieslySpider(scrapy.Spider):
    ...
    def start_requests(self):
        return [scrapy.FormRequest('http://series.ly/scripts/login/login.php',
                                   formdata={'lg_login': self.username,
                                             'lg_pass': self.password},
                                   callback=self.parse_main_page)]

    def parse_main_page(self, response):
        url = 'http://series.ly/@' + self.username + '/_series'
        return scrapy.Request(url, callback=self.parse_profile)

    def parse_profile(self, response):
        profile_uid = response.css('#profileUID').xpath('@value').extract()[0]
        yield scrapy.Request(
            'http://series.ly/perfil/subperfil/series.php?uid={}&id={}'
            .format(profile_uid, profile_uid),
